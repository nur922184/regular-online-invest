...
const AccountsList = () => {
  ...
  const fetchAccounts = async () => {
    try {
      const response = await fetch(`https://investify-api.duckdns.org/api/accounts/user/${user._id}`);
  ...
  const handleSetDefault = async (accountId) => {
    try {
      const response = await fetch(`https://investify-api.duckdns.org/api/accounts/set-default/${accountId}`, {
        method: 'PUT',
  ...
  const handleDelete = async (accountId) => {
    ...
    if (confirm.isConfirmed) {
      try {
        const response = await fetch(`https://investify-api.duckdns.org/api/accounts/delete/${accountId}`, {
          method: 'DELETE'
        });
