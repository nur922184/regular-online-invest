...
const SalaryBonus = () => {
  ...
  const getUserDepositStatus = async (userId) => {
    ...
    try {
      const txRes = await axios.get(`https://investify-api.duckdns.org/api/transactions/user/${userId}`);
  ...
  const loadBonusData = async () => {
    try {
      ...
      // সব ইউজার লোড
      const res = await axios.get("https://investify-api.duckdns.org/api/users/all");
  ...
  const claimBonus = async (level) => {
    ...
    try {
      const response = await axios.post("https://investify-api.duckdns.org/api/bonus/level-claim", {
        userId: user._id,
        level: level,
