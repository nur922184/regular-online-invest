...
const SalaryBonus = () => {
  ...
  const getUserDepositStatus = async (userId) => {
    ...
    try {
      const txRes = await axios.get(`https://107.167.94.212/api/transactions/user/${userId}`);
  ...
  const claimBonus = async (level) => {
    ...
    try {
      const response = await axios.post("https://107.167.94.212/api/bonus/level-claim", {
        userId: user._id,
        level: level,
