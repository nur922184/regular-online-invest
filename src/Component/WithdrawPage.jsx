...
const WithdrawPage = () => {
  ...
  const checkUserHasPaidInvestment = async () => {
    ...
    try {
      ...
      const res = await fetch(`https://107.167.94.212/api/investments/user/${user._id}`, {
        headers: {
  ...
  const loadAccounts = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`https://107.167.94.212/api/accounts/user/${user._id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`https://107.167.94.212/api/withdrawals/request`, {
        method: "POST",
        headers: { 
